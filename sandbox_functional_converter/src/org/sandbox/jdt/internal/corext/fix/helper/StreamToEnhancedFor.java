...
public class StreamToEnhancedFor extends AbstractFunctionalCall<ASTNode> {
	...
	public void rewrite(UseFunctionalCallFixCore useExplicitEncodingFixCore, ASTNode visited,
		...
		ASTEnhancedForRenderer renderer = new ASTEnhancedForRenderer(cuRewrite.getAST(), cuRewrite.getASTRewrite());
		renderer.renderReplace(info.model(), info.forEachStatement(), info.bodyStatements(), group);
	}
